= Rocky Packer Template Examples
:experimental:
...
That's why is it _dropped_ into the first partition of the first disk for *_BIOS Machines_*.
...
----
$ packer build -var-file="rocky-8-uefi.pkrvars.hcl" rocky-8-uefi-x86_64.pkr.hcl
----

== Rocky 8 UEFI Example
...
$ packer build -var-file="rocky-8-uefi.pkrvars.hcl" -var "ssh_password=$password" -var="disk_root_size=32G" -only="qemu.qemu" rocky-8-uefi-x86_64.pkr.hcl
----
