...
variable "boot_command_prefix" {
  type    = string
  default = "<wait><wait><wait><esc><wait><wait><wait>"
}
...
variable "iso_checksum" {
  type    = string
  default = "8488abc1361590ee7a3c9b00ec059b29dfb1da40f8ba4adf293c7a30fa943eb2"
...
variable "iso_external_mirror" {
  type    = string
  default = "cdimage.debian.org"
}

variable "iso_external_mirror_path" {
  type    = string
  default = "cdimage/release/current/amd64/iso-cd"
}
...
variable "use_default_display" {
  type    = bool
  default = true
  description = "Use QEMU default display"
}
...
locals {
  boot_command     = [
    "${var.boot_command_prefix}", "/install.amd/vmlinuz ",
    "initrd=/install.amd/initrd.gz ", "auto=true ",
  ...
  preseed_vars     = {
                       apt_cache_url        = "${var.apt_cache_url}"
                       country              = "${var.country}"
