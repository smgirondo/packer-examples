...
variable "iso_checksum" {
  ...
  description = "http://ftp.rediris.es/mirror/CentOS/7.9.2009/isos/x86_64/sha256sum.txt"
}
...
#  ]
  boot_command     = [
    "c<wait>",
    "linuxefi /images/pxeboot/vmlinuz ${var.install_boot_method} ",
    "${var.install_boot_ks}=http://{{ .HTTPIP }}:{{ .HTTPPort }}/${var.preseed_file}<wait> ",
    "${var.install_boot_options}<enter><wait>",
